...
void Msap_blStatus(const app_lib_otap_remote_status_t * bl_status, waps_item_t * item)
{
    /* Write indication over IPC message */
    msap_remote_status_ind_t * ind = &item->frame.msap.remote_status_ind;
    Waps_item_init(item,
                   WAPS_FUNC_MSAP_REMOTE_STATUS_IND,
                   sizeof(msap_remote_status_ind_t));
    item->frame.sfid = 0;
    ind->queued_indications = 0;
    ind->source = Addr_to_Waddr(bl_status->source);
    ind->num_bytes = bl_status->num_bytes;
    ind->crc = bl_status->crc;
    ind->seq = bl_status->seq;
    ind->type = bl_status->type;
    ind->status = bl_status->status;
    ind->processed_num_bytes = bl_status->processed_num_bytes;
    ind->processed_crc = bl_status->processed_crc;
    ind->processed_seq = bl_status->processed_seq;
    ind->area_id = bl_status->area_id;
    ind->major_version = bl_status->major_version;
    ind->minor_version = bl_status->minor_version;
    ind->maint_version = bl_status->maint_version;
    ind->devel_version = bl_status->devel_version;
    ind->update_req_timeout = bl_status->update_req_timeout;
}
...
static bool remoteStatus(waps_item_t * item)
{
    app_lib_settings_role_t role = APP_LIB_SETTINGS_ROLE_SINK;
    msap_remote_status_e result = MSAP_REMOTE_STATUS_SUCCESS;
    app_res_e role_res = APP_RES_OK;

    if (item->frame.splen != sizeof(msap_remote_status_req_t))
    {
        return false;
    }
    role_res = lib_settings->getNodeRole(&role);
    /* Check that remote status feature is permitted */
    if (!LockBits_isFeaturePermitted(LOCK_BITS_MSAP_REMOTE_STATUS))
    {
        result = MSAP_REMOTE_STATUS_ACCESS_DENIED;
    }
    else if (lib_state->getStackState() != APP_LIB_STATE_STARTED)
    {
        result = MSAP_REMOTE_STATUS_INVALID_STATE;
    }
    else if ((app_lib_settings_get_base_role(role) !=
             APP_LIB_SETTINGS_ROLE_SINK) || (role_res != APP_RES_OK))
    {
        result = MSAP_REMOTE_STATUS_INVALID_ROLE;
    }
    else
    {
        msap_remote_status_req_t * req = &item->frame.msap.remote_status_req;
        if (lib_otap->sendRemoteStatusReq(req->target) != APP_RES_OK)
        {
            result = MSAP_REMOTE_STATUS_OUT_OF_MEMORY;
        }
    }
    /* Build response */
    Waps_item_init(item,
...
static bool remoteUpdate(waps_item_t * item)
{
    msap_remote_update_req_t * req = &item->frame.msap.remote_update_req;
    app_lib_settings_role_t role = APP_LIB_SETTINGS_ROLE_SINK;
    msap_remote_update_e result = MSAP_REMOTE_UPDATE_SUCCESS;
    app_res_e role_res = APP_RES_OK;
    ...
    role_res = lib_settings->getNodeRole(&role);
    /* Check that remote update feature is permitted */
    if (!LockBits_isFeaturePermitted(LOCK_BITS_MSAP_REMOTE_UPDATE))
    {
        result = MSAP_REMOTE_UPDATE_ACCESS_DENIED;
    }
    else if (lib_state->getStackState() != APP_LIB_STATE_STARTED)
    {
        result = MSAP_REMOTE_UPDATE_INVALID_STATE;
    }
    else if ((app_lib_settings_get_base_role(role) !=
                APP_LIB_SETTINGS_ROLE_SINK) || (role_res != APP_RES_OK))
    {
        result = MSAP_REMOTE_UPDATE_INVALID_ROLE;
    }
    else if ((req->seq == 0) || (req->seq == 255))
    {
        result = MSAP_REMOTE_UPDATE_INVALID_SEQ;
    }
    else
    {
        switch(lib_otap->sendRemoteUpdateReq(req->target,
                                             req->seq,
                                             req->reboot_delay))
        {
            case APP_RES_OK:
                result = MSAP_REMOTE_UPDATE_SUCCESS;
                break;
            case APP_RES_INVALID_VALUE:
                result = MSAP_REMOTE_UPDATE_INVALID_REBOOT_DELAY;
                break;
            case APP_RES_INVALID_STACK_STATE:
                result = MSAP_REMOTE_UPDATE_INVALID_STATE;
                break;
            case APP_RES_RESOURCE_UNAVAILABLE:
                result = MSAP_REMOTE_UPDATE_OUT_OF_MEMORY;
                break;
            default:
                result = MSAP_REMOTE_UPDATE_ACCESS_DENIED;
                break;
    ...
    switch(lib_sleep->sleepStackforTime
           ...
           item->frame.msap.sleep_start_req.app_config_nrls))
        {
            ...
            case APP_RES_INVALID_CONFIGURATION:
                ...
                break;
            case APP_RES_INVALID_VALUE:
                result = MSAP_REMOTE_STATUS_INVALID_VALUE;
                break;
            default:
                result = MSAP_REMOTE_UPDATE_ACCESS_DENIED;
                break;
