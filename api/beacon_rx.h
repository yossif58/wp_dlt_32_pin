...
/**
 * \brief  Advertising channels to use, for app_lib_beacon_rx_start()
 */
typedef enum
{
...
/**
 * \brief  BTLE structure received from network
 */
typedef struct
{
    /** PDU type (BluteTooth Core_v5.0.pdf, p2567 */
    uint8_t     type;
 ...
 */
typedef void
    (*app_lib_beacon_rx_data_received_cb_f)(const app_lib_beacon_rx_received_t * packet);
 ...
 * \param   channel mask
 *          BluteTooth advertisement channel mask to use
 * \returns Result code, \ref APP_RES_OK if successful,
 *          APP_RES_INVALID_STACK_STATE if BLE Scanner is already running
 *          APP_RES_INVALID_CONFIGURATION if node is not LowLatency Headnode
 *          APP_RES_INVALID_NULL_POINTER if no callback set
 */
typedef app_res_e
 ...
 * \returns Result code, \ref APP_RES_OK if successful,
 *          APP_RES_INVALID_STACK_STATE if BLE Scanner is not running
 */
typedef app_res_e
 ...
 *
 * Example on use. When device receives a packet to destination endpoint 10, it
 * checks whether BLE scanner is active. If not, it will start it.
 *
 * @code
 *
 ...
 * @endcode
 *
 * \returns Result code, \ref true if running,
