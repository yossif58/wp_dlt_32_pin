...
/**
 * \brief   Function to stack sleep start.
 * \param   seconds
 *          Wirepas Mesh stack sleep time in seconds. Sleep time starts when the
 ...
 *          used radio.
 *          If set to 0 stack sleep is started for maximum period of 0x93A80
 *          seconds (7 days).
 * \param   appconf_wait_s
 *          Wait time in seconds for app config data from network before NRLS
 *        stack sleep.
 *          0 = App conf is not awaited before going to sleep.
 ...
 *          the maximum value. Returns \ref APP_RES_INVALID_STACK_STATE is stack
 *          is not started and node role and seconds is correct. Returns \ref
 *          APP_RES_OK when stack sleep is started.
 * \note    If the time when waking up from NRLS sleep and going back to NRLS
...
/**
 * \brief   Same functionality as in app_lib_sleep_stack_for_time_f without node
 *          role check. To be used only for testing purposes.
 */
typedef
app_res_e (*app_lib_sleep_allroles_for_time_f) (uint32_t seconds,
                                                uint32_t appconf_wait_s);

/**
 * \brief   Function to wakeup stack from sleep.
 *
 * Starts the Wirepas Mesh stack wakeup from sleep if application wants to
 * wake-up the Wirepas Mesh stack from NRLS sleep before NRLS sleep time has
 * elapsed.
 *
...
/**
 * \brief   Returns state of Wirepas Mesh stack sleep.
...
/**
 * \brief   Returns remining sleep time.
 *
 * Returns remaining Wirepas Mesh stack NRLS sleep time in seconds. Time period
...
/**
 * \brief   Function to set callback function which to be called before stack
 *          wakeup.
 *
 * Function to set callback function which to be called before stack wakeup from
 * sleep.
 * \param   callback
 *          Callback function
 */
...
/**
 * \brief   Returns latest time what used before Wirepas Mesh enters to sleep
 *        after NRLS sleep request given.
 *
 ...
 * \return  Total time used including application callbacks during that period.
 *        seconds
 */
