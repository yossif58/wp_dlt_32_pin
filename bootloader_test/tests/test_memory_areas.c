...
static bool check_memory_pattern(bl_memory_area_id_t id,
                                 ...
                                 uint8_t pattern)
{
    ...
    while (size_to_read > 0)
    {
        ...
        res = m_mem_services_p->startRead(id, m_read_buffer, pos, to_read);
        if (res != BL_RES_OK)
        {
        ...
        while (m_mem_services_p->isBusy(id))
        {
        }
...
static bool erase_memory(bl_memory_area_id_t id,
                         bl_memory_area_info_t * area_info_p)
{
    bl_interface_res_e res;
    ...
    {
        res = m_mem_services_p->startErase(id, &sector_base, &number_of_sector);
        if (res != BL_RES_OK)
        {
            Print_printf("ERROR: cannot startErase for area id %x "
                    "res=%d number_of_sector=%d\n", id, res, number_of_sector);
            return false;
        }
...
static bool write_memory_pattern(bl_memory_area_id_t id,
                                 bl_memory_area_info_t * area_info_p,
                                 uint8_t pattern)
{
    bl_interface_res_e res;
    ...
    /* Write chunk size is less than a page and a multiple of write_alignment.*/
    do
    {
        write_chunk_size /= 2;
    ...
    while (size_to_write > 0)
    {
        ...
        uint32_t next_page = ((uint32_t)pos &
                              (0xFFFFFFFF - (area_info_p->flash.write_page_size - 1))) +
        ...
        /* Write memory */
        res = m_mem_services_p->startWrite(id, pos, m_write_buffer, to_write);
        if (res != BL_RES_OK)
        {
...
static bool test_area(bl_memory_area_id_t id, bl_memory_area_info_t * area_info_p)
{
    bool res;
    ...
    res = erase_memory(id, area_info_p) &&
          check_memory_pattern(id, area_info_p, 0xff);
    ...
    res = write_memory_pattern(id, area_info_p, 0x15) &&
          check_memory_pattern(id, area_info_p, 0x15);
