 ...
 * \note    These do not correspond to NVIC priority levels, however, a lower
...
/**
 * Radio hardware magic number. This number is returned by the
 * \ref app_lib_system_get_radio_info_f "lib_system->getRadioInfo"() function
 * and can be used by the application to detect which radio platform it is
 * running on.
 */
typedef enum
{
 ...
 * spacing, output power levels, etc. This number is returned by the
...
typedef enum
{
    ...
    APP_LIB_SYSTEM_PROTOCOL_PROFILE_915_MHz_BRAZIL_SILABS
} app_lib_system_protocol_profile_e;
/**
 * Information returned by the \ref app_lib_system_get_radio_info_f
 * "lib_system->getRadioInfo"() function.
 */
typedef struct
{
...
/**
 * The startup callback. This callback is called after every interrupt, until
 * the stack is started by calling the \ref app_lib_state_start_stack_f
...
/**
 * The shutdown callback. This callback is called when the \ref
...
/**
 * Periodic callback. This function is called after a set delay. If the set
 * delay is zero, this function is called as soon as possible. If the stack is
 * not running, the callback is called after every interrupt, just like the
 * startup callback \ref app_lib_system_startup_cb_f.
 *
 ...
 * \note    The callback will be called once after returning from
 ...
 * 10000 (10 ms). Otherwise, returns \ref APP_RES_OK.
 *
 * Because of limited range of return value (32 bits) and the internal
 * comparison made by the scheduler, the maximum delay before being scheduled
 * cannot be higher than the value returned by the \ref
 ...
 * \note    When the callback returns \ref APP_LIB_SYSTEM_STOP_PERIODIC,
...
/**
 * Enter a critical section. Interrupts are disabled for the duration of the
 ...
 * \return  Result code, Always \ref APP_RES_OK
 * \note    Disabling the deep sleep mode will increase the power consumption
 ...
 *          application or \ref APP_RES_INVALID_CONFIGURATION if no interrupt
 *          handler or table set
 * \note    Fast interrupts and normal interrupts are
...
/**
 * Enable a platform-specific interrupt irq_n with given handler and priority.
 *
 ...
 * bootloader. As of v3.2 stack firmware release, the bootloader is at version
 * 3. For platforms without bootloader, this function returns 0.
 *
