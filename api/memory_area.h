...
/**
 * Start a block read from a memory area. Reading is asynchronous and \ref
 * app_lib_mem_area_isBusy_f "lib_memory_area->isBusy" must be checked for
...
/**
 * Start a block write to a memory area. Writing to a flash memory takes time
 * (5ms typical for a page). The call to the function must be asynchronous,
 * meaning that the write to the memory area is accepted and will succeed but
 * MAY not be finished. \ref app_lib_mem_area_isBusy_f "lib_memory_area->isBusy"
 * must be used to check if the write operation has terminated.
 *
 * \param  id
 ...
 *         \to and \p amount must be aligned on write_alignement field of
...
/**
 * Start the erase of one or multiple sectors in the memory area. Erasing is
 * asynchronous and \ref app_lib_mem_area_isBusy_f "lib_memory_area->isBusy"
 * must be checked for operation completion.
 *
 * \param  id
...
/**
* This function returns the list of areas defined in the bootloader and
...
/**
 * The function table returned from \ref app_global_functions_t.openLibrary
 */
 typedef struct
 {
